...
import com.greymax.android.sve.BuildConfig;
...
public class EglUtil {
    ...
    * To compile this code, we need this method.
    * */
    public static int loadShader(final String strSource, final int iType) {
        int[] compiled = new int[1];
        // create a vertex shader type (GLES20.GL_VERTEX_SHADER)
        // or a fragment shader type (GLES20.GL_FRAGMENT_SHADER)
		// create a new shader object with a call to glCreateShader() and store the ID of that object in iShader
        int iShader = GLES20.glCreateShader(iType);
        // add the source code to the shader and compile it
        GLES20.glShaderSource(iShader, strSource);
        GLES20.glCompileShader(iShader);
		// retrieving the compilation status
		// This tells OpenGL to read the compile status associated with shaderObjectId and write it to the 0th element of compiled.

		// This is another common pattern with OpenGL on Android. To retrieve a value,
		// we often use arrays with a length of 1 and pass the array into an OpenGL
		// call. In the same call, we tell OpenGL to store the result in the array’s first
		// element.
        GLES20.glGetShaderiv(iShader, GLES20.GL_COMPILE_STATUS, compiled, 0);
        if (compiled[0] == 0) {
			// we can get a human-readable message by calling glGetShaderInfoLog(shaderObjectId). 
			// If OpenGL has anything interesting to say about our shader, it will store the message in the shader’s info log.
    ...
    * */
    ...
    public static FloatBuffer toFloatBuffer(final float[] data) {
        final FloatBuffer buffer = ByteBuffer
                ...
                .asFloatBuffer();
        // add the coordinates to the FloatBuffer and set the buffer to read the first coordinate
